     ...
     def process_embeddings(
         ...
         try:
             ...
             metadata = {
                 ...
                 "num_additional_embeddings": len(additional_embeddings) if additional_embeddings els 
 0
             }
         ...
         try:
             ...
             if prompt_embeds is not None:
                 if negative_prompt_embeds is None and do_classifier_free_guidance:
                     raise ValueError("negative_prompt_embeds must be provided if using prompt_embeds 
 with guidance")
                 return {
             ...
             with torch.inference_mode():
                 ...
                 if do_classifier_free_guidance:
                     ...
                     pooled_prompt_embeds = torch.cat([negative_pooled_prompt_embeds,
             ...
             with torch.inference_mode(), torch.amp.autocast(device_type=self.device.type):
                 ...
                 if clip_skip is not None:
                     ...
                     if clip_skip > len(outputs.hidden_states):
                         raise ValueError(f"clip_skip ({clip_skip}) must be less than number of       
 layers")
                     text_embeds = outputs.hidden_states[-clip_skip]
                 else:
                     # Use default hidden state
                     text_embeds = outputs.hidden_states[-2] if output_hidden_states else
 outputs.last_hidden_state
                 ...
                 if self.debug:
                     logger.debug("CLIP encoding complete", extra={
                         ...
                         'pooled_shape': tuple(pooled_output.shape) if pooled_output is not None else 
 None
                     })
